    ...
    ** the function is using global variables:
    ...
    with lite.connect(db_name + '.sqlite') as con:
        ...
        for t, d in refGene.items():
            ...
            values = tuple([t] + d[2:6] + GeneID + [d[6], ensID] + [ucsc_acc.get(ensID, [None, None])[1] ,p_info[pr][0]])
            ...
            if GeneID[0] not in geneSet:
                ...
                if len(MGI) == 0:
                    MGI = [None]
                values = tuple(GeneID) + g_info[tnov][0:2] + tuple(d[0:2] + MGI + [gene_con.get(GeneID[0], None)]) + tuple([specie],)
