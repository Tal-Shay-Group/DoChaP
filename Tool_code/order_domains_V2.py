...
def order_domains(region, dTypeDict, dTypeID, dExt, dNames, dCDD):
    ...
    for pref in range(len(external)):
        if region[4] is None:
            if region[3].lower().startswith(external[pref]):
                ...
                currExt = region[4]
            break
        elif "Provisional" in region[4] and re.sub(r'\d+$', '', region[2]) in ['CHL', 'MTH', 'PHA', 'PLN', 'PTZ',
                                                                               'PRK']:
            currExt = region[2]
            break
        else:
    ...
    for pref in range(len(external)):
        if currExt is None:
            ...
            pref = 9
    if currExt is not None and currExt not in dExt:
        ...
        if region[2] not in dNames:
            ...
            if region[5] == None or region[5] not in dCDD:
            ...
            elif region[5] is not None and region[5] in dCDD:
                existExt = list(dTypeDict[dCDD[region[5]]][4:])
                currReg = dCDD[region[5]]
                if existExt[pref] is None or currExt in existExt[pref]:
                ...
                if region[3].lower() not in dTypeDict[currReg][2].lower() and dTypeDict[currReg][2].lower() not in \
                        region[3].lower():
                    ndesc = dTypeDict[currReg][2] + '; ' + region[3]
                else:
                    ndesc = dTypeDict[currReg][2]
                cdname = dTypeDict[currReg][0]
                oname = dTypeDict[currReg][1]
                if pref == 0:
                    cdname = region[2]
                elif region[2].lower() not in dTypeDict[currReg][0].lower() and region[2].lower() not in \
                        dTypeDict[currReg][1].lower():
            ...
            if pref == 0:
                cdname = region[2]
            elif region[2].lower() not in dTypeDict[currReg][0].lower() and region[2].lower() not in dTypeDict[currReg][
                1].lower():
                oname = dTypeDict[currReg][1] + '; ' + region[2]
            # dTypeDict[currReg] = (cdname, oname, ndesc, ncdd,) + tuple(existExt)
            dExt[currExt] = currReg
    elif currExt in dExt and currExt is not None:
        # print('extID exist')
        ...
        if region[2].lower() == dTypeDict[currReg][0].lower() or region[2].lower() in dTypeDict[currReg][1].lower():
            ...
            oname = dTypeDict[currReg][1]
            if region[2].lower() not in dTypeDict[currReg][0].lower() and region[2].lower() not in dTypeDict[currReg][
                1].lower():
        ...
        if region[2] in dNames:
            ...
            if region[5] not in dCDD and region[5] != None:
                ...
                currReg = dCDD[region[5]]
                if re.sub(r'[_ ]\d+$', '', region[2]) == re.sub(r'[_ ]\d+$', '', dTypeDict[currReg][0]) or re.sub(
                        r'[_ ]\d+$', '', region[2]) == re.sub(r'[_ ]\d+$', '', dTypeDict[currReg][1]):
                    ...
                    cdname = re.sub(r'[_ ]\d+$', '', dTypeDict[currReg][0])
                    oname = dTypeDict[currReg][1]
                    if region[2].lower() not in dTypeDict[currReg][0].lower() and region[2].lower() not in \
                            dTypeDict[currReg][1].lower():
                        oname = oname + '; ' + region[2]
                    existExt = list(dTypeDict[currReg][4:])
                    dNames[region[2]] = currReg
