...
def order_domains(region, dTypeDict, dTypeID, dExt, dNames, dCDD):
    ...
    for pref in range(len(external)):
        if region[4] == '' and region[3].lower().startswith(external[pref]):
            currExt = region[3]
            break
    ...
    if currExt not in dExt and currExt != '':
        ...
        if region[2] not in dNames:
            ...
            #dTypeDict[currReg] = (cdname, oname, ndesc, ncdd,) + tuple(existExt)                              
            dExt[currExt] = currReg
    elif currExt in dExt and currExt != '':
        #print('extID exist')
        ...
        if region[2] in dNames: 
            ...
            if region[5] not in dCDD and region[5] != '':
                dTypeID += 1
                ncdd = region[5]
                oname = region[2]
                cdname = region[2]
    ...
    return currReg, dTypeDict, dTypeID, dExt, dNames, dCDD