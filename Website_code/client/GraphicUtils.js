...
function runGenesCreation(result, ignorePredictions, preferences) {
    ...
    if (preferences != undefined && preferences.colorByLength != undefined) {
        colorForLengthDict = getColorForLength(result, ignorePredictions);
        preferences.colorByLength=colorForLengthDict; //change from boolean to list
    }
...
function createGraphicInfoForGene(gene, ignorePredictionsT, preferences) {
    ...
    ansGene.transcripts = [];
    ansGene.gene_symbol = gene.gene_symbol.toUpperCase().substring(0, 1) + gene.gene_symbol.toLowerCase().substring(1);
    ...
    */
    colorStyleByLength = undefined;
    if (preferences != undefined && preferences.colorByLength != undefined) {
        colorStyleByLength = preferences.colorByLength;
    }
...
function createGraphicInfoForTranscript(transcript, startCoordinate, endCoordinate, maxProteinLength, geneExons, specie, proteinStart, proteinEnd) {
    ...
    for (var i = 0; i < domains.length; i++) {
        ...
        ansTranscript.domains[i].source = domains[i].ext_id;
...
function getColorForLength(result, ignorePredictions) {
    ...
    var currSizeColor = 0;
    var interval=10;
    ans[currSizeColor]=getcolorFromList(colorArr)[0];
